...
class Home extends Component
{
    public function render()
    {
        ...
        $exclusiveProduct = Product::where([['is_exclusive',1]])->where(function($query){
            $query->where('status',1)->orWhere('status',0) ;
        })->orderBy('id','DESC')->take(16)->get();
        $trendingProduct = Product::where([['is_trending','!=',NULL]])->where(function($query){
            $query->where('status',1)->orWhere('status',0) ;
        })->orderBy('id','DESC')->take(16)->get();

        $bestProduct = Product::where([['is_best_sellers',1]])->where(function($query){
            $query->where('status',1)->orWhere('status',0) ;
        })->orderBy('id','DESC')->take(16)->get();
